		...
		if (value != nil) {
			...
			switch (typeEncoding[0]) {
				case '@':
				{
                    ...
                    if ([class isSubclassOfClass:[NSNull self]] == NO && [value isKindOfClass:[NSNull self]]) {
                        value = nil;
                    } else if ([class isSubclassOfClass:[NSString self]] && [value isKindOfClass:[NSNumber self]]) {
						value = [value stringValue];
					} else if ([class isSubclassOfClass:[NSNumber self]] && [value isKindOfClass:[NSString self]]) {
						// If the ivar is an NSNumber we really can't tell if it's intended as an integer, float, etc.
						NSNumberFormatter *numberFormatter = [[NSNumberFormatter alloc] init];
						[numberFormatter setNumberStyle:NSNumberFormatterDecimalStyle];
						value = [numberFormatter numberFromString:value];
					} else if ([class isSubclassOfClass:[NSDate self]] && [value isKindOfClass:[NSString self]] && (dateFormatter != nil)) {
